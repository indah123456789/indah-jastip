...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['update']) && !isset($_POST['delete'])) {
    ...
    // Dapatkan harga dari tabel jkttujuan
    $stmt = $pdo->prepare("SELECT harga FROM jkttujuan WHERE tujuan = :tujuan");
    ...
    if ($tujuanData) {
        ...
        // Insert data ke tabel jastipjkt
        $stmt = $pdo->prepare("INSERT INTO jastipjkt (tujuan, nama_pengirim, no_hp, jumlah_paket, tgl_pengirim, berat, biaya_paket, total_biaya) 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update'])) {
    ...
    // Dapatkan harga dari tabel jkttujuan untuk menghitung ulang biaya_paket dan total_biaya
    $stmt = $pdo->prepare("SELECT harga FROM jkttujuan WHERE tujuan = :tujuan");
    $stmt->execute([':tujuan' => $tujuan]);
    $tujuanData = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if ($tujuanData) {
        ...
        if ($jumlah_paket !== false && $berat !== false) {
            $stmt = $pdo->prepare("UPDATE jastipjkt SET 
                                    nama_pengirim = :nama_pengirim, 
                                    no_hp = :no_hp, 
                                    tujuan = :tujuan, 
                                    jumlah_paket = :jumlah_paket, 
...
if (isset($_GET['confirm_delete']) && isset($_GET['id'])) {
    ...
    $stmt = $pdo->prepare("DELETE FROM jastipjkt WHERE id = :id");
    $stmt->execute([':id' => $id]);
...
// Query untuk mengambil semua data dari tabel jastipjkt
$sql = "SELECT id, tujuan, nama_pengirim, no_hp, jumlah_paket, tgl_pengirim, berat, biaya_paket, total_biaya FROM jastipjkt";
